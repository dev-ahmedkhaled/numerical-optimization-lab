...
# Unconstrained: f(x,y) = x^2 + 2y^2 - 2xy    → min at (0,0), f=0 (Rosenbrock-like)
...
# ============================================================
# 2. STEEPEST DESCENT (exact line search via Armijo / quadratic)
# ============================================================
...
def plot_convergence(histories, labels, title, f, xlim=(-1,5), ylim=(-1,5)):
    ...
    for i, (hist, lbl) in enumerate(zip(histories, labels)):
        arr = np.array(hist)
        ax1.plot(arr[:,0], arr[:,1], '-o', markersize=4,
                 color=colors[i % len(colors)], label=lbl, alpha=0.85, linewidth=1.5)
        ax1.plot(arr[0,0], arr[0,1], 's', color=colors[i % len(colors)], markersize=8)
    ax1.set_xlabel('x'); ax1.set_ylabel('y')
    ax1.set_title('Optimization Paths'); ax1.legend(fontsize=9)
    ax1.set_xlim(xlim); ax1.set_ylim(ylim)

    # Convergence
    for i, (hist, lbl) in enumerate(zip(histories, labels)):
        iters = np.arange(len(hist))
        fvals = [f(h) for h in hist]
        ax2.semilogy(iters, fvals, color=colors[i % len(colors)],
                     label=lbl, linewidth=2)
    ax2.set_xlabel('Iteration'); ax2.set_ylabel('f(x) [log scale]')
    ax2.set_title('Convergence History'); ax2.legend(fontsize=9)
...
if __name__ == "__main__":
    ...
    xopt, fopt, nit, hist_gd = gradient_descent(f1, grad_f1, x0, alpha=0.4)
    ...
    xopt2, fopt2, nit2, hist_bfgs = bfgs(f2, grad_f2, x0)
