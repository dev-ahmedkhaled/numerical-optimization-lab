...
def hess_f2(xy):
    return np.array([[2.0, -2.0], [-2.0, 4.0]])
...
def combined_constraints():
    ...
    result = minimize(
        lambda v: (v[0] - 1) ** 2 + (v[1] - 2) ** 2 + (v[2] - 3) ** 2,
        x0=[1.0, 1.0, 4.0],
        constraints=[
...
def plot_convergence(
    histories, labels, title, f, f_star=0.0, xlim=(-1, 5), ylim=(-1, 5)
):
...
def plot_lagrange():
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(13, 5))
    fig.suptitle(
        "Lagrange Multipliers — min x²+y²  s.t.  x+y=4",
        fontsize=14,
        fontweight="bold",
        color="#1E2761",
    )
    ...
    # Place label above the line (offset perpendicular: (-0.3, 0.3) direction)
    ax1.text(
        3.8 + 0.15,
        3.8 - 0.15,
        "∇f = (4,4)",
    ...
    # Place label below the line (offset perpendicular: (0.3, -0.3) direction)
    ax1.text(
        3.2 + 0.15,
        3.2 - 0.15,
        "∇g = (1,1)",
    ...
    for r in [1, 2, 3, 4]:
        circle = plt.Circle(
            (0, 0), r, fill=False, color="blue", alpha=0.5, linewidth=1.5
        )
        ax2.add_patch(circle)
    ax2.plot(x_line, y_line, "r--", linewidth=2.5, label="x + y = 4")
    circle_tangent = plt.Circle(
        (0, 0),
        np.sqrt(8),
        fill=False,
        color="blue",
...
def plot_kkt():
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(13, 5))
    fig.suptitle(
        "KKT Conditions — min x²+y²  s.t.  x+y ≥ 4",
        fontsize=14,
        fontweight="bold",
        color="#1E2761",
    )
    ...
    ax2.text(
        5,
        0.8,
        "All 4 KKT conditions satisfied → x*=(2,2) is optimal",
        fontsize=11,
        fontweight="bold",
        ha="center",
        color="darkgreen",
...
def plot_combined():
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(13, 5))
    fig.suptitle(
        "Combined Constraints — min (x-1)²+(y-2)²+(z-3)²  s.t.  x+y+z=6,  x≥1",
        fontsize=13,
        fontweight="bold",
        color="#1E2761",
    )
    ...
    ax1.axvline(
        x=1, color="red", linewidth=2.5, linestyle="--", label="x = 1 (boundary)"
    )
    ax1.fill_betweenx(
        [-0.5, 4], 1, 4, alpha=0.15, color="green", label="Feasible: x ≥ 1"
    )
    ax1.plot(
        1,
        2,
...
if __name__ == "__main__":
    ...
    print_result("x*", xopt)
    print_result("f(x*)", fopt)
    print_result("Iterations", nit, "d")
    ...
    print_result("x*", xopt2)
    print_result("f(x*)", fopt2)
    print_result("Iterations", nit2, "d")
    ...
    print_result("x*", res_lag.x)
    print_result("f*", res_lag.fun)
    ...
    fig1.savefig("imgs/unconstrained_convergence.png", dpi=150, bbox_inches="tight")
    ...
    fig2.savefig("imgs/quasi_newton_convergence.png", dpi=150, bbox_inches="tight")
    print("  [2/5] quasi_newton_convergence.png")

    # Plot 3: Lagrange Multipliers
    fig3 = plot_lagrange()
    fig3.savefig("imgs/lagrange_multipliers.png", dpi=150, bbox_inches="tight")
    print("  [3/5] lagrange_multipliers.png")

    # Plot 4: KKT Conditions
    fig4 = plot_kkt()
    fig4.savefig("imgs/kkt_conditions.png", dpi=150, bbox_inches='tight')
    print("  [4/5] kkt_conditions.png")

    # Plot 5: Combined Constraints
    fig5 = plot_combined()
    fig5.savefig("imgs/combined_constraints.png", dpi=150, bbox_inches='tight')
